...
void FEHL(void (*F)(double t1,double * y1,double * dy1),int NEQN,
...

{
	...
	for  (K=1;K<=NEQN;K++)
		...
		5643.0*F4[K-1]))+(41040.0*F1[K-1]-28352.0*F2[K-1]));
	F(*T+(*H)/2.0,F1,F5);
	CH=(*H)/7618050.0;
	for  (K=1;K<=NEQN;K++)
		S[K-1]=Y[K-1]+CH*((902880.0*YP[K-1]+(3855735.0*F3[K-1]-
		1371249.0*F4[K-1]))+(3953664.0*F2[K-1]+277020.0*F5[K-1]));
}



void RKFS(void (*F)(double t1,double * y1,double * dy1),int NEQN,
		  ...
		  int *KFLAG)
{
	int HFAILD=0,OUTPUT=0;
	double A=0,AE=0,DT=0,EE=0,EEOET=0,ESTTOL=0,ET=0,HMIN=0,REMIN=0,
		RER=0,S=0,SCALE=0,TOL=0,TOLN=0,U26=0,EPSP1=0,EPS=0,YPK=0;
	double a=0.2;

	int K=0,MAXNFE=0,MFLAG=0;
...
l65: *INIT=1;
	*H=fabs(DT);
	...
	for (K=1;K<=NEQN;K++)
	{
		...
		TOLN=TOL;
		YPK=fabs(YP[K-1]);
		if(YPK*pow((*H),5)>TOL) *H=pow((TOL/YPK),a);
	}
l70:
	if(TOLN<=0.0) *H=0.0;
...
l80: *H=SIGN(*H,DT);
	...
	*IFLAG=7;
	return;
l85: if(fabs(DT)>U26*fabs(*T))goto l95;
	for (K=1;K<=NEQN;K++)
	{
...
l100: HFAILD=0;
	...
	*H=DT;
	goto l200;
l150: *H=0.5*DT;
l200: if(*NFE<=MAXNFE)goto l220;
	*IFLAG=4;
	*KFLAG=4;
	return;
l220:  FEHL(F,NEQN,Y,T,H,YP,F1,F2,F3,F4,F5,F1);
	*NFE=*NFE+5;
	EEOET=0.;
	for (K=1;K<=NEQN;K++)
	{
		...
		*KFLAG=5;
		return;
l240: EE=fabs((-2090.*YP[K-1]+(21970.*F3[K-1]-15048.*F4[K-1]))
...
l250:
	...
	OUTPUT=0;
	S=0.1;
	if(ESTTOL<59049.)S=0.9/pow(ESTTOL,a);
	*H=S*(*H);
...
l270:
	...
	*NFE=*NFE+1;
	S=5.;
	if(ESTTOL>1.889568E-4)S=0.9/pow(ESTTOL,a);
